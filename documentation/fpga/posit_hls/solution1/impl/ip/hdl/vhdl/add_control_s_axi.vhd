...
architecture behave of add_control_s_axi is
    ...
    constant ADDR_IN_A_DATA_0  : INTEGER := 16#10#;
    constant ADDR_IN_A_CTRL    : INTEGER := 16#14#;
    constant ADDR_IN_B_DATA_0  : INTEGER := 16#18#;
    constant ADDR_IN_B_CTRL    : INTEGER := 16#1c#;
    constant ADDR_OUT_C_DATA_0 : INTEGER := 16#20#;
    constant ADDR_OUT_C_CTRL   : INTEGER := 16#24#;
    ...
    -- internal registers
    signal int_in_a            : UNSIGNED(31 downto 0) := (others => '0');
    signal int_in_b            : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_IN_A_DATA_0 =>
                        rdata_data <= RESIZE(int_in_a(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    in_a                 <= STD_LOGIC_VECTOR(int_in_a);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_IN_A_DATA_0) then
                    int_in_a(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_in_a(31 downto 0));
                end if;
            end if;
        end if;
    end process;
