...
main (int argc, char *argv[])
{
  ...
  height = atoi (argv[4]);
  float *image1, *image2;
  image1 = malloc (sizeof (float) * width * height);
  image2 = malloc (sizeof (float) * width * height);
  open_image (argv[1], image1, width, height);
  open_image (argv[2], image2, width, height);

  image_correlation (image1, image2, width, height, 555, 222);
...
//       driver_inverse_gauss_jordan (inve_matrix_gauss_jordan, i, a, b,
//       &bench); conversion_into_double (b, c_device, _matrix_size_2);
